...
<table style="border-bottom:1px solid #ccc;">
	<tr >
		...
		<td><h4 class="titulo">REPORTES PERI&Oacute;DICOS DE CLASES PROGRAMADAS (REALIZADAS Y NO REALIZADAS)</h4></td>
	</tr>
</table>
<br>
				...
				<?php if (isset($registros)): ?>
					<?php foreach ($registros as $key => $detalle): ?>
						<tr>
							...
							<td><?php echo !empty($detalle['Sala']['TIPO_SALA'])?utf8_encode($detalle['Sala']['TIPO_SALA']):utf8_encode($detalle['SalaReemplazo']['TIPO_SALA']); ?></td>
							<td><?php echo date('H:i', strtotime($detalle['ProgramacionClase']['HORA_INICIO'])).' a '.date('H:i', strtotime($detalle['ProgramacionClase']['HORA_FIN'])); ?></td>
