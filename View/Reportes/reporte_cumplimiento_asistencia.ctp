<div class="row">
	<div class="col-md-12">
		<h2 style="border-bottom: 1px solid #0c253d; padding-bottom: 5px;"><label>Cumplimiento de Registro de Asistencia Docente</label></h2>				
	</div>
</div>
<div class="row m-t-20">
	<form 
		id="form-filtro-basico" 
		action="<?php echo $this->Html->url(array('action'=>'grillaCumplimientoAsistencia')); ?>" 
		method="POST">
		<div class="col-md-2">
		...
		<div class="col-md-2">
			<div class="form-group">
				<label for="">Rut Docente <span title="Se desplegar&aacute; un listado de opciones coincidentes con su b&uacute;squeda." data-toggle="tooltip"><i class="fa fa-info-circle"></i></span></label>
				<input
					class="form-control autocompletable-input" 
					type="text"
					data-field-au="Docente.RUT"
					name="data[Filtro][docente_rut]"
					style="text-transform:uppercase;" 
					autocomplete="off" >
			</div>  
		</div>
		<div class="col-md-2">
			<div class="form-group">
				...
				<input 
					...
					name="data[Filtro][docente_nombre]"
					style="text-transform:uppercase;" 
					autocomplete="off"
				>
				<input 
	...
	$('.autocompletable-input').each(function(index, el) {
		...
		$(this).autocomplete({
			...
			select: function( event, ui ) {
				if (variable_filtro=='Docente.NOMBRE' || variable_filtro=='Docente.RUT') {
