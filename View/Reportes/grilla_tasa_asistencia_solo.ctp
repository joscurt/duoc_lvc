				...
				#debug($indicadores_alumnos);exit();
		  			foreach ($registros as $key => $value): 
						...
						if (isset($indicadores_alumnos[$value['AlumnoAsignatura']['COD_HORARIO_ASIGNATURA']][$value['Alumno']['COD_ALUMNO']])) {
							// debug($value['AsignaturaHorario']['CLASES_REGISTRADAS']);exit();
							...
							// $asistidas = $indicadores_alumnos[$value['AlumnoAsignatura']['COD_HORARIO_ASIGNATURA']][$value['Alumno']['COD_ALUMNO']]['CLASES_PRESENTE'];
							// $no_asistidas = $value['AsignaturaHorario']['CLASES_REGISTRADAS'] - $asistidas;

							// $porcentaje = $asistidas/($asistidas+$no_asistidas)*100;
		...
		<div class="form-group">
			<form id="form-exportar" method="POST" target="_blank">
				...
				<button 
					...
					href="<?php echo $this->Html->url(array('action'=>'excelTasaAsistenciaSolo')); ?>"
					><i class="fa fa-file-excel-o"></i>&nbsp;Exportar Excel</button>
				<button 
					class="btn btn-success btn-exportable" 
					href="<?php echo $this->Html->url(array('action'=>'pdfTasaAsistenciaSolo')); ?>"
					><i class="fa fa-file-pdf-o"></i>&nbsp;Exportar PDF</button>
				<button 
					class="btn btn-success btn-exportable" 
					target="_blank"
					href="<?php echo $this->Html->url(array('action'=>'imprimirTasaAsistenciaSolo')); ?>"
